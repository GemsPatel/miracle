...
function getTemplateDetailAndSendMail($template_key,$data = array())
{
	...
	{	
	  ...
	  //parse template Contect and returns a string
	  ...
	  //parse template Subject and returns a string
	  $subject = $CI->parser->parse_string( $template_detail['template_subject'], $data, TRUE );
	  $subject = _pwu( $subject ); // parsing with some predefined configuration
	  
	  echo "subject: ".$subject."<br>";
	  echo "message: ".$message;die;
	  sendMail( $data['email_address'], $subject ,$message ); // send mail
	}
	else
