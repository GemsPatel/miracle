 ...
 */
function setLoginSessions($customer_id,$customer_group_type,$customer_emailid, $is_reInstantiatedSession=false)
{
	...
	$CI->session->set_userdata($data); //set session store in user details
	...
	if( $customer_group_type != 'C' )
	{
		...
		if( !$is_reInstantiatedSession )
			saveLogins( $customer_id , $customer_group_type);
	}
...
* @abstract function will set all sessions related to login and perform other login related activity
*/
function saveLogins( $cust_admin_user_id , $l_user_type)
{
	$CI =& get_instance();

	$data['cust_admin_user_id'] = $cust_admin_user_id;
	$data['sessions_id'] = $CI->session->userdata('sessions_id');
	...
	$dataIp['sessions_id'] = $CI->session->userdata('sessions_id');
 ...
 */
function reInstantiateSession( $session_id )
{
	if( !empty( $session_id ) )
	{
		...
		{
			//login lib
			loadLib( 'Login/login-fun' );
				
			set_userdata( array( 'sessions_id'=> $row["sessions_id"] ) );
			setLoginSessions($row["customer_id"], $row["customer_group_type"], $row["customer_emailid"], true); 
				
			setFlashMessage( "warning", "Your App session has been refreshed by server." );
