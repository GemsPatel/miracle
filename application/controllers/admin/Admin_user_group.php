...
class Admin_user_group extends CI_Controller {
	...
	var $is_ajax = false;
	var $cAutoId = 'admin_user_group_id';
	var $cPrimaryId = '';
	var $cTable = 'admin_user_group';
	var $controller = 'admin_user_group';
	var $is_post = false;
	var $per_add = 1;
	var $per_edit = 1;
	var $per_delete = 1;
	...
	{
		parent::__construct();
		$this->load->model('admin/mdl_admin_user_group','adusr');
		$this->adusr->cTableName = $this->cTable;
		$this->adusr->cAutoId = $this->cAutoId;
		$this->is_ajax = $this->input->is_ajax_request();
		
		if($this->input->get('item_id') != '' || $this->input->post('item_id') != '')
			$this->cPrimaryId  = $this->adusr->cPrimaryId = _de($this->security->xss_clean($_REQUEST['item_id']));
...
*/	
	...
	function adminUserGroupKey($str)
	{
		if($this->cPrimaryId)
			$this->db->where($this->cAutoId." !=",$this->cPrimaryId);
				
		$c = $this->db->where('admin_user_group_key',$str)->get($this->cTable)->num_rows();
		if($c > 0)
		{
			$this->form_validation->set_message('adminUserGroupKey', getErrorMessageFromCode( '01026' ) );
			return false;
		}
		else
			return true;
	...
	function adminUserGroupName($str)
	{
		if($this->cPrimaryId)
			$this->db->where($this->cAutoId." !=",$this->cPrimaryId);
			
		$c = $this->db->where('admin_user_group_name',$str)->get($this->cTable)->num_rows();
		if($c > 0)
		{
			$this->form_validation->set_message('adminUserGroupName', getErrorMessageFromCode( '01025' ) );
			return false;
		}
		else
		  return true;
	...
	{
		$logType = 'V';
		saveAdminLog($this->router->class, 'Admin User Group ', $this->cTable, $this->cAutoId, 0, $logType);
		if($this->per_view != 0)
		{
...
*/
	function adminUserGroupForm()
	{
		...
		$data = array();
		$this->form_validation->set_rules('admin_user_group_name','Admin user group name','trim|required|callback_adminUserGroupName');
		$this->form_validation->set_rules('admin_user_group_key','Admin user group key','trim|required|callback_adminUserGroupKey');
		...
		else
		{
			...
			{
				$this->adusr->cPrimaryId = $this->cPrimaryId; // setting variable to model
				$this->adusr->saveData();
