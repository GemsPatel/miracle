...
class Mdl_admin_user_group extends CI_Model
{
	...
	function getData()
	{
		if($this->cPrimaryId == "")
		{
			$f = $this->input->get('f');
			$s = $this->input->get('s');
			$admin_user_group_name = $this->input->get('admin_user_group_name_filter');
			
			if(isset($admin_user_group_name) && $admin_user_group_name != "")
			    $this->db->where('admin_user_group_name LIKE \'%'.$admin_user_group_name.'%\' ');
...
*/	
	function deleteData($ids)
	{
		...
		if($ids)
		{
			...
			$res = $this->db->query('SELECT u.admin_user_id FROM admin_user u INNER JOIN admin_user_group a ON u.admin_user_group_id = a.admin_user_group_id AND a.admin_user_group_id ="'.$id.'"');
			$res = $res->num_rows();
